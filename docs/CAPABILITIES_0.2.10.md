# HearthCrew 0.2.10 能力矩阵

由 protocol/action-capabilities.json 生成。参数与执行能力存在不代表所有真实地形已验收。

| 动作 | 必需参数 | 允许参数 | 当前行为与限制 |
|---|---|---|---|
...
| MINE | position | position, count, resource, accessBudget | position为起点，count默认1最多64块；resource可限定起始方块种类。连续采掘同类连通自然方块，起点附近8格、身体起点32格内；不能挖脚下支撑；仅在授权accessBudget内开挖安全自然通路。回执分别列出requested、broken、acquired及pendingDrops；已破坏但未拾取时恢复回收，不重挖空气。 自动从整个背包选适用工具，无需先SELECT；数量任务优先提交所需批次而非逐块调用。缺工具先查真实配方和自有材料，准备工作仍属当前阶段。 accessBudget默认16，0..64；接近与回收共享，同一SEQUENCE的所有步骤共享根预算，子步骤0可禁止其开路。不可破坏保护建筑、容器、危险地形或自动搭桥。 |
...
| PICKUP | target | target, count, accessBudget | 使用真实世界目标；接受不等于完成。 不传count（仅兼容旧count=0）。 accessBudget默认16，0..64；接近与回收共享，同一SEQUENCE的所有步骤共享根预算，子步骤0可禁止其开路。不可破坏保护建筑、容器、危险地形或自动搭桥。 |
...
| COLLECT_RESOURCE | resource, count, candidates | resource, count, candidates, position, radius, accessBudget | 目标物品resource、新增数量count 1..64、候选自然方块candidates 1..16；position为范围中心，radius 1..32默认32。本动作限矿物/石材候选；原木必须使用GATHER完成自然树验证。仅采这些方块，不隐式开路。缺通路时另行明确EXCAVATE预算。实际原版拾取事件累计，非背包总量。 accessBudget默认16，0..64；接近与回收共享，同一SEQUENCE的所有步骤共享根预算，子步骤0可禁止其开路。不可破坏保护建筑、容器、危险地形或自动搭桥。 |
| SEQUENCE | actions | actions, accessBudget | actions包含1..8个{kind,parameters}已确定的准备步骤，不嵌套SEQUENCE，不含长期FOLLOW/GUARD或安全反射。逐步核验，失败即停，已完成步骤不重放。 accessBudget默认16，0..64；接近与回收共享，同一SEQUENCE的所有步骤共享根预算，子步骤0可禁止其开路。不可破坏保护建筑、容器、危险地形或自动搭桥。 |
| YIELD | 无 | 无 | 主动让路：执行器选择附近2..4格安全站位，带空间租约。无位置或动态阻挡则有限等待后说明原因；不传坐标，不改地形。 |

## 共通边界

- 执行协议3；默认安全开路16块，范围32格，不拆保护建筑、容器或自行搭桥。根SEQUENCE共享预算，重规划不续额。
- 拾取遵守原版实际接触、延迟与背包规则；本人、队友与未回收证据分开。未观测不等于消失。
- 32格近查、128格已加载地表线索；探索成功核对终点及新增区域。
- 常规模型上下文24KiB；分页证据通过 observe(contextRef,cursor) 读取。交流回合不能操控身体。
- 五个角色工具和三个独立Luna/high保留；没有固定生存脚本或库存补偿。
- [本版修复、检查边界与手测清单](RELEASE_0.2.10.md)。
